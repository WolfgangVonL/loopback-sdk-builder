...
module.exports = function generate(ctx) {
  ...
  **/
  ...
  function buildModelProperties(model, isInterface) {
    ...
    // Add Computed Model Properties
    const modelMixins = ctx.app.models[model.name].definition.settings.mixins;
    const computedProperties = (modelMixins && modelMixins.Computed) ? modelMixins.Computed.properties : [];
    Object.keys(computedProperties).forEach((propertyName) => {
      let property = computedProperties[propertyName];
      let isOptional = isInterface && !property.required ? '?' : '';
      let defaultValue = !isInterface ? ` = ${buildPropertyDefaultValue(property)}` : '';
      defaultValue = ctx.defaultValue !== 'enabled' && ctx.defaultValue !== 'strict' ? '' : defaultValue;
      defaultValue = ctx.defaultValue === 'strict' && !property.hasOwnProperty('default') ? '' : defaultValue;
      output.push(`  "${propertyName}"?: ${buildPropertyType(property)}${defaultValue};`);
    });
    // Add Model Relations
